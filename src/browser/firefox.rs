...
pub async fn extract_cookies(config: &BrowserCookieConfig) -> Result<CookieStore> {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        Err(RurlError::Unsupported(
            "Firefox cookie extraction is only implemented for macOS".to_string(),
        ))
    }
}
...
mod macos {
    ...
    pub fn extract_cookies(config: &BrowserCookieConfig) -> Result<CookieStore> {
        ...
        let cookie_db = newest_path(find_cookie_dbs(&search_roots)).ok_or_else(|| {
            RurlError::FileNotFound("Firefox cookies database not found".to_string())
        })?;
        ...
        let mut stmt = conn.prepare(&query).map_err(|e| {
            RurlError::BrowserCookie(format!("Failed to prepare Firefox query: {}", e))
        })?;
    ...
    fn resolve_container(
        ...
        let containers_path = cookie_db
            ...
            .ok_or_else(|| {
                RurlError::BrowserCookie("Firefox profile path not found".to_string())
            })?;
        ...
        let data = fs::read_to_string(&containers_path).map_err(|e| {
            RurlError::BrowserCookie(format!("Failed to read containers.json: {}", e))
        })?;
